...
export const SecureTokenDisplay = () => {
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 p-3 bg-white/80 backdrop-blur-sm rounded-lg border border-warm-brown-200">
        <Loader2 className="h-4 w-4 animate-spin text-warm-brown-600" />
        <span className="text-sm text-warm-brown-700">Loading tokens...</span>
      </div>
    );
  }
